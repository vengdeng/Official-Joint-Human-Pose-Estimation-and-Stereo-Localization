...
class PifPaf(object):
    def __init__(self, stride, seed_threshold,
                 ...
                 fixed_b=None, psf=False,stereo_vote = True):
        self.stride = stride
        ...
        self.psf = psf
        self.pif_nn = 16
        self.paf_nn = 1 if connection_method == 'max' else 1
        self.stereo_vote = stereo_vote
    def __call__(self, fields):

        if self.profile is not None:
            self.profile.enable()
        psf = fields[2]
        connects = self.normalize_psf(*psf[0][0])
        if self.stereo_vote:
            targets_sf, scales_sf = self.vote_connection(connects, 0.3)
            core_sf = self.vote_connection(connects, 0.3,core_only= True)
        else:
            targets_psf = 0
            core_psf = 0
        ann_results = {}
        ...
        for tt in range(2):
            if self.stereo_vote:
                targets_psf = targets_sf[:, tt]
                core_psf = core_sf[:, tt]
            pif, paf = fields[tt][self.head_indices[0]], fields[tt][self.head_indices[1]]
            ...
            gen = PifPafGenerator(
                pif, paf, targets_sf=targets_psf,core_sf = core_psf,
                stride=self.stride,
            ...
            ann_results[tt] = annotations
            gens[tt] = gen
        connection_candidate = self.score_psf_target(connects,0.01,0.05,gens)
        return ann_results,connection_candidate

    def vote_connection(self, connects, v_th, core_only=False):
        targets = np.zeros((connects.shape[0], connects.shape[1],
                            int(connects.shape[3] * self.stride),
                            int(connects.shape[4] * self.stride)),dtype=np.float32)
        scales = np.zeros_like(targets)
        ns = np.zeros_like(targets)
        for i in range(2):
            pif = connects[:, i]
            for t, p, scale, n in zip(targets[:, i], pif, scales[:, i], ns[:, i]):
                v, x, y, s = p[:, p[0] > v_th]
                x = x * self.stride
                y = y * self.stride
                s = s * self.stride
    ...
    def normalize_psf(self, intensity_fields, j1_fields, j2_fields, j1_fields_logb, j2_fields_logb, *,
                      fixed_b=None):
        ### normalize the voting value which will help decide the threshold
        for t in intensity_fields:
            t -= t.mean()
            t /= t.max()
        ...
        psf = np.stack((j1_fields3, j2_fields3), axis=1)

        return psf

    def score_psf_target(self,pafsss, pifhr_floor=0.7, score_th=0.1,gens = []):
        scored_connection = []
        for c, fourds in enumerate(pafsss):
            ...
            if pifhr_floor < 1.0:
                # ij_b = np.round(fourds[0, 1:3] * 8).astype(np.int)
                # ij_b[0] = np.clip(ij_b[0], 0, gens[0]._pifhr.shape[2] - 1)
                # ij_b[1] = np.clip(ij_b[1], 0, gens[0]._pifhr.shape[1] - 1)
                # pifhr_b = gens[0]._pifhr[j1i, ij_b[1], ij_b[0]]
                scores_b = scores
            else:
                scores_b = scores
            mask_b = scores_b > score_th
            scored_connection.append(np.concatenate((
                np.expand_dims(scores_b[mask_b], 0),
                fourds[1, 1:4][:, mask_b],
                fourds[0, 1:4][:, mask_b],
            )))

        return scored_connection


class PifPafGenerator(object):
    def __init__(self, pifs_field, pafs_field, *, targets_sf, core_sf,
                 stride,
                 seed_threshold,
                 connection_method,
                 pif_nn,
                 paf_nn,
                 skeleton,
                 debug_visualizer=None,stereo_v = False):
        self.pif = pifs_field
        self.paf = pafs_field
        ...
        self.targets_sf = targets_sf
        self.core_sf = core_sf
        self._pifhr, self._pifhr_scales = self._target_intensities()
        if stereo_v:
            w = 0.92
        else:
            w = 1
        self._pifhr = w * self._pifhr + (1-w) * targets_sf
        self._pifhr_core = self._target_intensities(core_only=True)
        self._pifhr_core = self._pifhr_core * w + (1-w)*core_sf
        if self.debug_visualizer:
