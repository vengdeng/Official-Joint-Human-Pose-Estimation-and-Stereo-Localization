...
class Preprocess(metaclass=ABCMeta):
    @abstractmethod
    def __call__(self, image,image2, anns,anns2, meta=None):
...
class Normalize(Preprocess):
    ...
    def __call__(self, image,image2, anns,anns2, meta=None):
        ...
        anns2 = self.normalize_annotations(anns2)
        ...
        return image,image2, anns,anns2, meta
...
class Compose(Preprocess):
    ...
    def __call__(self, image,image2, anns,anns2, meta=None):
        for p in self.preprocess_list:
            image,image2, anns,anns2, meta = p(image,image2, anns,anns2, meta)
...
class RescaleRelative(Preprocess):
    ...
    def __call__(self, image,image2, anns,anns2, meta=None):
        if meta is None:
            image,image2, anns,anns2, meta = Normalize()(image,image2, anns,anns2)
        else:
            meta = copy.deepcopy(meta)
        anns = copy.deepcopy(anns)
        anns2 = copy.deepcopy(anns2)
        ...
        image,image2, anns,anns2, scale_factors = self.scale(image,image2, anns,anns2, scale_factor)
    ...
    def scale(self, image,image2, anns,anns2, factor):
        # scale image
        w, h = image.size
        target_size = (int(w * factor), int(h * factor))
        image = image.resize(target_size, self.resample)
        image2 = image2.resize(target_size, self.resample)
        # rescale keypoints
        ...
        for ann1 in anns2:
            ...
            ann1['keypoints'][:, 0] = (ann1['keypoints'][:, 0] + 0.5) * x_scale - 0.5
            ann1['keypoints'][:, 1] = (ann1['keypoints'][:, 1] + 0.5) * y_scale - 0.5
            ann1['bbox'][0] *= x_scale
            ann1['bbox'][1] *= y_scale
            ann1['bbox'][2] *= x_scale
            ann1['bbox'][3] *= y_scale

        return image,image2, anns,anns2, np.array((x_scale, y_scale))
...
class Crop(Preprocess):
    ...
    def __call__(self, image,image2, anns,anns2, meta=None):
        ...
        image,image2, anns,anns2, ltrb = self.crop(image,image2, anns,anns2)
        meta['offset'] += ltrb[:2]
    ...
    def crop(self, image,image2, anns,anns2):
        w, h = image.size
        ...
        for ann2 in anns2:
            ...
            ann2['keypoints'][:, 0] -= x_offset
            ann2['keypoints'][:, 1] -= y_offset
            ann2['bbox'][0] -= x_offset
            ann2['bbox'][1] -= y_offset


        return image,image2, anns,anns2, np.array(ltrb)
...
class CenterPad(Preprocess):
    ...
    def __call__(self, image,image2, anns,anns2, meta=None):
        ...
        image,image2, anns, anns2, ltrb = self.center_pad(image,image2, anns,anns2)
        meta['offset'] += ltrb[:2]
    ...
    def center_pad(self, image,image2, anns, anns2):
        w, h = image.size
        ...
        image2 = torchvision.transforms.functional.pad(
            image2, ltrb, fill=(124, 116, 104))
        # pad annotations
        for ann in anns:
            ...
            ann['bbox'][1] += ltrb[1]
        for ann2 in anns2:
            ann2['keypoints'][:, 0] += ltrb[0]
            ann2['keypoints'][:, 1] += ltrb[1]
            ann2['bbox'][0] += ltrb[0]
            ann2['bbox'][1] += ltrb[1]

        return image, image2, anns,anns2, ltrb
...
class HFlip(Preprocess):
    ...
    def __call__(self, image,image2, anns,anns2, meta=None):
        ...
        for ann2 in anns2:
            ...
            ann2['keypoints'][:, 0] = -ann2['keypoints'][:, 0] - 1.0 + w
            if self.swap is not None:
                ann2['keypoints'] = self.swap(ann2['keypoints'])
                # meta['horizontal_swap'] = self.swap
            ann2['bbox'][0] = -(ann2['bbox'][0] + ann2['bbox'][2]) - 1.0 + w
